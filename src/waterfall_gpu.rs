use crate::hardware::{ReceiveStreamDescriptorPtr, StreamMessage};
...
pub struct WaterfallGpu {
    texture_groups: HashMap<ReceiveStreamDescriptorPtr, TextureGroup>,
...
impl WaterfallGpu {
    ...
    pub fn add_row(&mut self, msg: &StreamMessage, device: &Device, queue: &Queue) {
        let key = msg.receive_stream_descriptor_ptr.clone();
        ...
        group.add_row(device, queue, msg);
    }
    ...
    pub fn draw_list(&self) -> impl Iterator<Item = ChunkDrawInfo> {
        ...
        self.texture_groups
            .iter()
            .flat_map(|(receive_stream_descriptor_ptr, group)| {
                group
                    .finished_textures
                    .iter()
...
impl ActiveTexture {
    pub fn new(device: &Device, msg: &StreamMessage, prev_texture: Texture) -> Self {
...
impl TextureGroup {
    fn add_row(&mut self, device: &Device, queue: &Queue, msg: &StreamMessage) {
        if self.active_texture.current_row >= TEXTURE_HEIGHT as usize {
            self.swap_active_texture(device, queue, msg);
        }
    ...
    fn swap_active_texture(&mut self, device: &Device, queue: &Queue, msg: &StreamMessage) {
        if self.active_texture.current_row < TEXTURE_HEIGHT as usize {
